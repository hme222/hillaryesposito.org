...
## Designpowers Integration

This skill is bundled with [Designpowers](https://github.com/Owl-Listener/designpowers). When Designpowers is active:

**When to invoke:** After `design-strategy` and `design-taste` are complete, before or alongside `ui-composition`. Use it to generate data-backed design system recommendations — then reconcile them with the brief, personas, and taste profile.

**Priority order when recommendations conflict:**
1. User direction (creative director overrides everything)
2. Designpowers taste profile (`~/.designpowers/taste-profile.md`)
3. Design brief and personas from `design-state.md`
4. UI UX Pro Max database recommendations

**Script path in Designpowers:** Run from this skill's base directory (provided when invoked):
...
When persisting a design system, write to the project root and record the path in `design-state.md`:

```bash
python scripts/search.py "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```

**Reconcile, don't copy blindly.** UI UX Pro Max may suggest palettes or styles that conflict with the taste profile or accessibility requirements. Apply Designpowers accessibility minimums from `ui-composition` — contrast, touch targets, reduced motion — even when the database suggests otherwise.

**Upstream:** [nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) (MIT). Update via `uipro update` or by replacing `skills/ui-ux-pro-max/`.
